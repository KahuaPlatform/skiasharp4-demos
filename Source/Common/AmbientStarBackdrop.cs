...
// Viewbox. Renders a deep-space gradient + slow drifting + twinkling parallax
// starfield. Shared base class — each demo declares a thin sealed wrapper in
// its own namespace so `<local:BackgroundSurface>` can be referenced from XAML.
//
// Animation is driven by the host MainPage calling Invalidate() each frame;
// this control tracks its own dt via Stopwatch.
public abstract class AmbientStarBackdrop : SKCanvasElement
{
    // Override these in subclasses if a game wants different background colors.
